...
def train_evaluate_and_log(pipeline, X_train, y_train, X_test, y_test, model_path, model_type, params=None,
    ...
    # --- NOUVEAU : Calcul des métriques industrielles ---
    overfit_pct = 0.0
    if rmse_cv > 0:  # Sécurité mathématique
        overfit_pct = (rmse_cv - rmse_train) / rmse_cv

    TOLERANCE = 0.30
    if overfit_pct > TOLERANCE:
        optuna_score = rmse_cv * (1 + overfit_pct)
    else:
        optuna_score = rmse_cv
        ...
        with mlflow.start_run(run_name=f"Optuna_Study_{args.model_type}") as parent_run:
            ...
            def objective(trial):
                ...
                with mlflow.start_run(run_name=f"trial_{trial.number}", nested=True):
                    ...
                    # --- B. Calcul de la pénalité d'Overfitting pour Optuna ---
                    # Pendant l'entraînement, on utilise le CV au lieu du Test comme référence
                    overfit_pct = (rmse_cv - rmse_train) / rmse_cv

                    # Logique industrielle avec pénalité
                    TOLERANCE = 0.30
                    if overfit_pct > TOLERANCE:
                        # On ajoute une forte pénalité proportionnelle à l'infraction
                        # Si RMSE=15 mais overfit=50%, le score devient 15 * 1.5 = 22.5
                        optuna_score = rmse_cv * (1 + overfit_pct)
                    else:
                        # Si le modèle est dans les normes, Optuna cherche la pure performance
                        optuna_score = rmse_cv
                    # --- C. Logging unifié ---
                    trial_metrics = {
                        "rmse_cv": rmse_cv,
                        ...
                        "rmse_test": rmse_test,
                        "overfit_pct": overfit_pct,  # On logue ce pourcentage pour le suivre
                        "optuna_score": optuna_score
                    }
                    ...
                    mlflow.log_metric("optuna_score", optuna_score)

                    # Optuna cherche maintenant à minimiser ce nouveau score
                    return optuna_score
            ...
            final_params["model_type"] = args.model_type
            ...
            train_evaluate_and_log(
                ...
                MODEL_OUTPUT_PATH, args.model_type,params=final_params, is_optimized=True
            )
